...
def input_actions():
    ...
    while True:
        try:
            ...
            try:
                if choice == 1:
                    hours, minutes = input("Enter hours and minutes separated "
                                           "by a space: ").split()
                    ...
                    size = int(input("Enter the size of downloading information "
                                     "in kb: "))
                    return choice, size
                else:
                    print('Enter the number in 1-3:')
    ...
    try:
        while values[choice] < limit:
            with concurrent.futures.ThreadPoolExecutor(th) as executor:
                end_random_list = []
                for i in range(0, th):
                    end_random_list.append(''.join(random.choice(
        ...
        print(f'good:{Fore.GREEN}{values[2]}{Style.RESET_ALL}, time:{Fore.GREEN}'
              f'{values[1]:.1f}{Style.RESET_ALL}, size:{Fore.GREEN}{values[3]}'
              f'{Style.RESET_ALL}')
    except (Exception, KeyboardInterrupt):
